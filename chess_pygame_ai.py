...
def slide_moves(
    board: Board, r: int, c: int, side: str, directions: List[Tuple[int, int]]
) -> List[Move]:
    moves: List[Move] = []
...
def draw_highlights(
    screen: pygame.Surface, selected: Optional[Tuple[int, int]], legal_moves: List[Move]
):
...
def render_info_panel(
    ...
    label = font_small.render(
        turn_text + (f"  {status_text}" if status_text else ""), True, (30, 30, 30)
    )
                ...
                if turn == "w":  # human's turn
                    ...
                    if selected is None:
                        ...
                        if piece != "." and is_white(piece):
                            ...
                            legal_for_selected = [
                                m for m in all_moves if m[0] == r and m[1] == c
                            ]
                        else:
                            selected = None
                            legal_for_selected = []
